...
class DBBackup extends MObj{
    ...
    public function render_data()
    {
        if($this->mPostBack)
        {
            ...
            {
                $content='';
                $link=$this->backup_table_txt("mcarticles.txt", "mcarticles");
                ...
                $content.='The mcmembership has been backup in the <a href="' . $link . '">' . $link . '</a><br />';

                foreach($this->mCategoryTypes as $category_type)
                {
                    $link=$this->backup_table_txt("mc".$category_type."dictionary.txt", "mc".$category_type."dictionary");
                    $content.='The mc'.$category_type.'dictionary has been backup in the <a href="' . $link . '">' . $link . '</a><br />';
                    $link=$this->backup_table_txt("mc".$category_type."s.txt", "mc".$category_type."s");
                    $content.='The mc'.$category_type.'s has been backup in the <a href="' . $link . '">' . $link . '</a><br />';
                }
            ...
            {
                $content='';
                $link=$this->backup_table_xml("mcarticles.xml", "mcarticles",
                    array("id", "title", "pages", "loc", "type", "year", "abstract", "comment", "keywords"),
                    "article");
                $content.='The mcarticles has been backup in the <a href="' . $link . '">' . $link . '</a><br />';
                $link=$this->backup_table_xml("mcmembership.xml", "mcmembership",
                ...
                foreach($this->mCategoryTypes as $category_type)
                {
                    $link=$this->backup_table_xml("mc".$category_type."dictionary.xml", "mc".$category_type."dictionary",
                        array("id", "detail", "parentid", "description"),
                        $category_type."dictionary");
                    $content.='The mc'.$category_type.'dictionary has been backup in the <a href="' . $link . '">' . $link . '</a><br />';
                    $link=$this->backup_table_xml("mc".$category_type."s.xml", "mc".$category_type."s",
